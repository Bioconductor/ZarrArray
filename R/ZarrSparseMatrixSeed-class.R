...
setClass("ZarrSparseMatrixSeed",
         ...
         representation(
           ...
           ## Absolute path to the Zarr file so the object won't break when
           ## the user changes the working directory (e.g. with 'setwd()').
           filepath="character",
           
           ## Name of the group in the Zarr file where the sparse matrix is
           ## stored.
           group="character",
...
setMethod("path", "ZarrSparseMatrixSeed", function(object) object@filepath)

### Just a placeholder for now. Doesn't actually allow changing the path of
### the object yet.
setReplaceMethod("path", "ZarrSparseMatrixSeed",
...
### - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
### Low-level internal zarrsparse data readers
###
...
.read_zarrsparse_dim <- function(filepath, group)
{
  if (zarrexists(filepath, paste0(group, "/shape"))) {
    ## 10x format
...
.read_zarrsparse_layout <- function(filepath, group)
{
  if (zarrexists(filepath, paste0(group, "/shape"))) {
    ## 10x format
    return("csr")
...
.read_zarrsparse_indptr <- function(filepath, group)
  read_zarrsparse_component(filepath, group, "indptr")

.read_zarrsparse_data <-
  function(filepath, group, subdata, start=NULL, count=NULL)
  {
    name <- .get_data_name(subdata)
    read_zarrsparse_component(filepath, group, name, start=start, count=count)
  }

### The row (or column) indices stored in Zarr dataset "indices" are 0-based
### and we return them as such.
.read_zarrsparse_indices <- function(filepath, group, start=NULL, count=NULL)
  read_zarrsparse_component(filepath, group, "indices",
...
{
  if (!zarrexists(filepath, group))
    stop(wmsg("Zarr group \"", group, "\" does not exist ",
              "in this Zarr file"))
  if (zarrisdataset(filepath, group)) {
    is_zarr_X_or_layer <- group == "/X" ||
      substr(group, 1L, 8L) == "/layers/"
    msg1 <- c("\"", group, "\" is an Zarr dataset, not an Zarr group, ",
              "so it looks like the matrix that you are trying to ",
              "access is not stored in a sparse format. Please ",
              "consider using the ")
    if (is_zarr_X_or_layer) {
      msg2 <- c("ZarrMatrix() constructor if you are trying ",
                "to access the central matrix of an h5ad file. ",
                "Otherwise, use the ZarrArray() constructor.")
    } else {
      msg2 <- "ZarrArray() constructor to access this dataset."
    }
    stop(wmsg(msg1, msg2))
  }
  if (!zarrisgroup(filepath, group))
...
.check_data_and_subdata <- function(filepath, group, subdata)
{
  ...
  if (!zarrexists(filepath, data_fullname))
    stop(wmsg("Zarr object \"", data_fullname, "\" does not ",
              "exist in this Zarr file. Are you sure that Zarr ",
              "group \"", group, "\" contains a sparse matrix ",
              "stored in CSR/CSC/Yale layout?"))
  if (is.null(subdata)) {
    if (zarrisgroup(filepath, data_fullname))
      stop(wmsg("\"", data_fullname, "\" is an Zarr group, not an ",
    ...
    if (!zarrisdataset(filepath, data_fullname))
    ...
    if (zarrisdataset(filepath, data_fullname))
      stop(wmsg("\"", data_fullname, "\" is an Zarr dataset, not an ",
    ...
    if (!zarrexists(filepath, subdata_fullname))
      stop(wmsg("Zarr object \"", subdata_fullname, "\" does not ",
                "exist in this Zarr file."))
    if (!zarrisdataset(filepath, subdata_fullname))
...
{
  if (is.null(dim)) {
    dim <- .read_zarrsparse_dim(filepath, group)
...
.get_sparse_matrix_layout <- function(filepath, group, sparse.layout=NULL)
{
  if (is.null(sparse.layout)) {
    zarrsparse_layout <- .read_zarrsparse_layout(filepath, group)
    ## Layout in R will be transposed w.r.t. layout used in h5 file.
    ans <- switch(zarrsparse_layout, `csr`="CSC", `csc`="CSR",
                  stop(wmsg("unsupported 'zarrsparse_layout': ",
                            zarrsparse_layout)))
    return(ans)
  }
                               ...
                               dim=NULL, sparse.layout=NULL)
{
  ...
  ## Zarr so:
  ## - "compressed sparse row" at the Zarr level translates
  ##   into "compressed sparse column" at the R level,
  ## - "compressed sparse column" at the Zarr level translates
                                         ...
                                         BPPARAM=NULL)
{
  ...
  if (DATABLOCKLEN == 0L || ans_nzcount <= DATABLOCKLEN) {
    ...
    ans_data <- .read_zarrsparse_data(x@filepath, x@group, x@subdata,
                                    start=start, count=count)
    ans_row_indices <- .read_zarrsparse_indices(x@filepath, x@group,
                                              start=start, count=count)
...
.extract_data_from_adjacent_cols <- function(x, j1, j2)
{
  ...
  ans_nzdata <- .read_zarrsparse_data(x@filepath, x@group, x@subdata,
                                    start=start, count=count)
...
.random_extract_nonzero_data_by_col <- function(x, j)
{
  ...
  data <- .read_zarrsparse_data(x@filepath, x@group, x@subdata, start=idx2)
...
setMethod("extractNonzeroDataByCol", "CSC_ZarrSparseMatrixSeed",
          function(x, j)
          {
...
setMethod("extractNonzeroDataByRow", "CSR_ZarrSparseMatrixSeed",
          function(x, i)
          {
